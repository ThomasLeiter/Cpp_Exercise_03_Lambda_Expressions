...
int main(){
    ...
    bool all_gt_three = std::all_of(vec.cbegin(), vec.cend(), [](int x){return x > 3;});
    ...
    bool any_gt_five = std::any_of(vec.cbegin(), vec.cend(), [](int x){return x > 5;});
    ...
    bool none_lt_zero = std::none_of(vec.cbegin(), vec.cend(), [](int x){return x < 0;});
    ...
    std::cout << "Squares: ";
    std::for_each(vec.cbegin(), vec.cend(), [](int x) {std::cout << x*x << " ";});
    std::cout << '\n';
    // Squares: 1 1 4 9 25 64 169

    // Use transform to map values of vec into container halves
    std::vector<double> halves;
    std::transform(vec.cbegin(),vec.cend(),std::back_inserter(halves),[](int x){return x / 2.0;});
    print_vector(halves);
    ...
    std::transform(vec.cbegin(),vec.cend(),std::back_inserter(fractions),[divisor](int x){return x / divisor;});
    print_vector(fractions);
    // [0.333333, 0.333333, 0.666667, 1, 1.66667, 2.66667, 4.33333]

    // Capture variable by reference
    int sum = 0;
    std::for_each(vec.cbegin(), vec.cend(), [&sum](int x){sum += x;});
    ...
    std::vector<double> fifth = map_int_to_double(vec, [](int x){return x/5.0;});
    print_vector(fifth);
    ...
    std::vector<double> added = map_with_function<int,double>(
        vec, [summand](int x){return x + summand;});
    print_vector(added);
